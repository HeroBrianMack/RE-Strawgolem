...
/**
 * When a choppable tree exists and the lumberjack's hand is empty, fetches an
 * axe from the bound chest.
 */
public class LumberFetchGoal extends Goal {
    private static final Predicate<ItemStack> AXE = s -> s.getItem() instanceof AxeItem;

    private final LumberjackGolem golem;
    private boolean done;

    public LumberFetchGoal(LumberjackGolem golem) {
        this.golem = golem;
    ...
    public boolean canUse() {
        return golem.getMainHandItem().isEmpty() && golem.hasDepositChest()
                ...
                && LumberChopGoal.findTreeBase(golem) != null
                && chestHas(AXE);
    }
