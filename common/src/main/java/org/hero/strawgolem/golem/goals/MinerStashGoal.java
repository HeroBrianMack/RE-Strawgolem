...
/**
 * Returns the pickaxe (or anything else in hand) to the bound chest once there
 * has been no mining work for a while.
 */
public class MinerStashGoal extends Goal {
    private static final int IDLE_BEFORE_STASH = 100;

    private final MinerGolem golem;
    private int idleTicks;

    public MinerStashGoal(MinerGolem golem) {
        this.golem = golem;
    ...
    public boolean canUse() {
        ...
        if (MinerMineGoal.holdingPickaxe(golem) && MinerMineGoal.findTargetBlock(golem) != null) {
            idleTicks = 0;
            return false;
        }
