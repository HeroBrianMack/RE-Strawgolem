...
/**
 * Returns whatever the lumberjack is holding to the bound chest once the
 * forest has been quiet for a while.
 */
public class LumberStashGoal extends Goal {
    private static final int IDLE_BEFORE_STASH = 100;

    private final LumberjackGolem golem;
    private int idleTicks;

    public LumberStashGoal(LumberjackGolem golem) {
        this.golem = golem;
    ...
    public boolean canUse() {
        ...
        if (LumberChopGoal.holdingAxe(golem) && LumberChopGoal.findTreeBase(golem) != null) {
            idleTicks = 0;
            return false;
        }
